...
BEGIN
  ...
  v_expected := upper(substr(encode(hmac(('KEY|' || v_plan_code || '|' || v_random)::bytea, v_secret::bytea, 'sha256'), 'hex'), 1, 8));
...
BEGIN
  ...
  v_sig := upper(substr(encode(hmac(('KEY|' || v_plan_code || '|' || v_random)::bytea, v_secret::bytea, 'sha256'), 'hex'), 1, 8));
